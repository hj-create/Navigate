(function () {
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    function appendMessage(text, from) {
      ...
      msg.dataset.from = from;              // for TTS selectors
      msg.classList.add('message');         // generic hook
...
(() => {
  if (window.__chatbotVoiceInit) return;
  window.__chatbotVoiceInit = true;
  ...
  const input = pick(['#chat-input', '.chat-input', 'textarea.chat-input']);
  const messages = pick(['#chat-messages', '.chat-messages', '.messages', '#thread', '.conversation']);
  if (!input) return;
  ...
  function textOf(node) {
    if (!node) return '';
    const botMsg = node.matches?.('.chat-bot, .bot, .assistant, [data-from="bot"], [data-role="bot"]')
      ? node
      : node.querySelector?.('.chat-bot, .bot, .assistant, [data-from="bot"], [data-role="bot"]');
    return (botMsg || node).textContent.trim();
  }

  function readLastBotMessage() {
    if (!messages) return;
    const nodes = messages.querySelectorAll('.chat-bot, .bot, .assistant, [data-from="bot"], [data-role="bot"]');
    ...
    const obs = new MutationObserver((muts) => {
      for (const m of muts) {
        for (const n of m.addedNodes) {
          ...
          const isBot = n.matches?.('.chat-bot, .bot, .assistant, [data-from="bot"], [data-role="bot"]') ||
                        n.querySelector?.('.chat-bot, .bot, .assistant, [data-from="bot"], [data-role="bot"]');
